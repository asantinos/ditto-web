...
In idle it collapses to a 38×9 px bar. Hovering expands it to show "Ditto" and the current shortcut; clicking the expanded pill starts recording. Recording uses a compact 104×28 px surface with a dedicated cancel button.
...
- **Matte** — off-white with compact voice-reactive bars. The default. Inspired by Apple's matte hardware finishes.
...
- **Matte and Onyx** show nine vertical bars in front of the pill, like a tiny EQ. Each bar maps to a frequency band from low to high; louder voice means taller bars.
...
Silero VAD decides when speech is present, RMS controls overall intensity, and frequency analysis shapes the individual bars or wave. This processing is visual only: it never trims or changes the audio sent to Whisper.
...
The pill is placed inside the work area of the chosen monitor, with a small margin from the edge so it doesn't overlap with the taskbar. Bottom positions sit 6 px above the usable screen edge.
